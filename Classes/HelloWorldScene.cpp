...
#define create_height (-_screenSize.height * 0.010)//下面的，想大改小

#define create_weight01 _screenSize.width * 0.0140//上面的
#define create_height01 _screenSize.height * 0.008

#define create_weight1 _screenSize.width * 0.0138
#define create_height1 (-_screenSize.height * 0.010)//下面的，想大改小

#define create_weight11 _screenSize.width * 0.0135//上面的
#define create_height11 _screenSize.height * 0.0185

#define create_weight2 _screenSize.width * 0.0022
#define create_height2 _screenSize.height * 0.000//下面的，想大改小

#define create_weight22 _screenSize.width * 0.0032//上面的
...
void  HelloWorldScene::initPhysics() {
	...
	tableBox.Set(b2Vec2(_screenSize.width * 0.54f / PTM_RATIO, (_screenSize.height - 100) * 0.02f / PTM_RATIO),
		b2Vec2(_screenSize.width * 0.95f / PTM_RATIO, (_screenSize.height - 100) * 0.02f / PTM_RATIO));
	tableBody->CreateFixture(&tableBox, 0);
	...
	tableBox.Set(b2Vec2(_screenSize.width * 0.54f / PTM_RATIO, (_screenSize.height - 100) * 0.98f / PTM_RATIO),
		b2Vec2(_screenSize.width * 0.95f / PTM_RATIO, (_screenSize.height - 100) * 0.98f / PTM_RATIO));
	tableBody->CreateFixture(&tableBox, 0);
	...
	p2[6] = (_screenSize.height - 100) * 0.09f;
	p1[7] = _screenSize.width * 1.00f;
	p2[7] = (_screenSize.height - 100) * 0.91f;
...
void HelloWorldScene::ballset() {//读取文件中球的位置以及杆的方向和是否出杆
	...
	else//否则进入通过给出白球和子球的位置，指示出进袋的路线
	{
		...
		float qiux = 0;
		float qiuy = 0;//要进袋的那个球
		//if (b2_0[0] < 410)
		//{
		//	qiux = b2_0[0] + create_weight;
		//	if (b3_0[0] > 330)
		//	{
		//		qiuy = 667 - (b3_0[0] + create_height);//要进袋的那个球

		//	}
		//	else {
		//		qiuy = 667 - (b3_0[0] + create_height01);//要进袋的那个球
		...
		//	if (b3_0[0] > 330)
		//	{
		//		qiuy = 667 - (b3_0[0] + create_height1);//要进袋的那个球

		//	}
		//	else {
		//		qiuy = 667 - (b3_0[0] + create_height11);//要进袋的那个球
		...
		//	if (b3_0[0] > 330)
		//	{
		//		qiuy = 667 - (b3_0[0] + create_height2);//要进袋的那个球

		//	}
		//	else {
		//		qiuy = 667 - (b3_0[0] + create_height22);//要进袋的那个球
		...
		qiuy = qiuy + 2 * ((b3_0[0] - doy) / (abs(b3_0[0] - doy))) * creat_y;//计算出需要击球的点
		point8.x = qiux;
		point8.y = qiuy;
		point9.x = b2_0[1];
		point9.y = b3_0[1];
...
void  HelloWorldScene::update(float dt) {
	...
	for (int j = 0; j < count; ++j) {
		...
		if (!ball->isVisible()) {//球进洞的情况
			...
			if ((ballPos1.x < _screenSize.width * 0.00f)|| (ballPos1.x > _screenSize.width ) || (ballPos1.y < (_screenSize.height - 100) * 0.03f) || (ballPos1.y > (_screenSize.height - 100) * 0.98f))//判断是否进球
			{
...
void HelloWorldScene::reset1() {
	...
	float  b2[100];//用来记录x坐标
	float b3[100];//用来记录y坐标
	int r1[100];//用来记录半径
	float b2_0[2];
	float b3_0[2];
	int do_id;
