package com.william.www.thestock.bean;
...
/*数据解析依照自己需求来定，如果服务器直接返回百分比数据，则不需要客户端进行计算*/
        ...
        for (int i = 0; i < count; i++) {
            String[] t = jsonArray.optString(i).split(" ");/*  "t[0]0930 t[1]9.50 t[2]4707",*/
    ...
    public void myParseMinutes(String result) {
        String[] temp=result.split("\"");
        String[] temp1=temp[1].split(";");

        for (int i = 0; i < temp1.length; i++) {
            String[] t = temp1[i].split(",");/*  [0]09:30,[1]6.91,[2]1577500,[3]10905487,[4]6.91,[5]6.93,[6]6.91 */
            if (i==0){
                baseValue = Float.parseFloat(t[1]);
            }
            MinutesBean minutesData = new MinutesBean();
            minutesData.time = t[0];
            minutesData.cjprice = Float.parseFloat(t[5]);
            minutesData.avprice = Float.parseFloat(t[1]);
            minutesData.cjnum = Integer.parseInt(t[2]);
            minutesData.total = Float.parseFloat(t[3]);
        ...
        System.out.println("实时数据:"+getDatas().size());
    }
    ...
    public void myParseKline(JSONArray jsonArray){
        ArrayList<KLineBean> kLineBeans = new ArrayList<>();
        if (jsonArray.length()!=0){
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONArray dayData = jsonArray.optJSONArray(i);
                KLineBean kLineData = new KLineBean();
                kLineBeans.add(kLineData);
                kLineData.date = dayData.optString(0);
                kLineData.open = (float) dayData.optDouble(6);
                kLineData.close = (float) dayData.optDouble(3);
                kLineData.high = (float) dayData.optDouble(4);
                kLineData.low = (float) dayData.optDouble(5);
                kLineData.vol = (float) dayData.optDouble(8);
                volmax = Math.max(kLineData.vol, volmax);
                xValuesLabel.put(i, kLineData.date);
            }
        }
